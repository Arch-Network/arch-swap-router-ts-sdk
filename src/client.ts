...
export function createRouterClient({ source, network = "testnet" }: RouterClientOptions): RouterClient {
  ...
  async function quote(request: QuoteExactInRequest | QuoteForOutputRequest, amount: bigint, forOutput: boolean): Promise<SwapQuote> {
    ...
    const clamm = clammStep && loadClamm(accounts, clammStep.inputMint === venues.clamm.tokenMintA, config);
